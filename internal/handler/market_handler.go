...
func (mh *marketHandler) PriceWebSocket(c *gin.Context) {
	...
	if err != nil {
		...
		conn.WriteJSON(model.WebSocketResponse{Message: "Failed to upgrade to WebSocket"})
		return
	}
	defer conn.Close()
	...
	for {
		...
		case t := <-ticker.C:
			{
				filterBy := model.WebSocketRequest{
					FilterBy:   "all",
				...
				if err != nil {
					...
					conn.WriteJSON(model.WebSocketResponse{Message: "Failed to process message from usecase"})
					continue
				}

				responseMessage := model.WebSocketResponse{
...
func (mh *marketHandler) Price(c *gin.Context) {
	markets, err := mh.marketUsecase.Price(model.WebSocketRequest{FilterBy: "all"})
	if err != nil {
...
func (mh *marketHandler) ConvertPrice(c *gin.Context) {
	var convertPrice *model.ConvertPrice
